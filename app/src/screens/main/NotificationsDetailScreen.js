...
const FEATURES = [
  "Real-time alert pings when a circle member is flagged",
  "Post-drive safety report summaries",
  "Circle activity updates and invites",
  "Alerts only — no spam",
];

function BellIcon({ size = 52, color }) {
  return (
...
export default function NotificationsDetailScreen({ navigation }) {
  const insets = useSafeAreaInsets();
  const [status, setStatus] = useState(null);

  useEffect(() => {
    Notifications.getPermissionsAsync().then(({ status: s }) => setStatus(s));
  }, []);
    ...
    <SafeAreaView style={styles.safe} edges={["top"]}>
      ...
      >
        ...
        <View style={styles.iconWrap}>
          <BellIcon size={52} color={Colors.brandBlue} />
        </View>

        <Text style={styles.title}>Notifications</Text>
        <Text style={styles.description}>
          Push notifications are used for real-time circle alerts and post-drive reports.
          The app never sends marketing or spam — you only hear from Notifeye when it matters.
        </Text>
